...
export class GoogleMapsService {
  ...
  async getDirections(start: CoodinateTypes, end: CoodinateTypes) {
    ...
    try {
      ...
      if (err instanceof InternalServerErrorException) throw new InternalServerErrorException("Internal Server Error")
    ...
    try {
      await this.redisClient.set(cacheKey, JSON.stringify(result), "EX", this.CACHE_TTL)
    } catch (err) {
