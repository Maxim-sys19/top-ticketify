...
export class RouteService {
  ...
  constructor(
    ...
    @InjectRepository(RouteTransport)
    private readonly routeCompanyTransportsRepository: Repository<RouteTransport>,
    private readonly googleMapService: GoogleMapsService,
    private readonly redisService: RedisService,
    private readonly attachCompanyAndTransports: AttachCompanyAndTransports,
  ) {
    ...
    try {
      ...
      if (existedRoute)
        throw new BadRequestException(
          `Route ${existedRoute.routeName} already exists`,
        );
      const response = await this.googleMapService.getDirections(
        createRouteDto.start,
        createRouteDto.end,
      );
      const route = new RouteTransport();
      route.start = createRouteDto.start;
      route.end = createRouteDto.end;
      route.routeName = createRouteDto.routeName;
      route.departureTime = createRouteDto.departureTime;
      route.arrivalTime = createRouteDto.arrivalTime;
      ...
      const attachedRoute =
        await this.attachCompanyAndTransports.attachTransportAndCompany(
          route,
          createRouteDto.company,
          createRouteDto.transports,
        );
      await this.routeCompanyTransportsRepository.save(attachedRoute);
      try {
  ...
  async update(id: number, updateRouteDto: UpdateRouteDto) {
    try {
      const { start, end, arrivalTime, departureTime, routeName } =
        updateRouteDto;
